...
class ScriptContainer {
  ...
  load(forceReload = false) {
    if (forceReload || !this.loaded) {
      ...
      if (this.envSupported) {
        ...
        if (_.has(this.scriptFuncs, 'init')) {
          const state = this.getState();
          this.callScriptFunc('init', state.getVMState());
        }
      }
    }
  }
  ...
  envCheck() {
    ...
    const supportedNodeEnvs = _.get(this.getScriptProp('config'), 'supportedNodeEnvs');
    ...
    if (this.envSupported) {
      ...
      if (state.validate()) {
        ...
        try {
          ...
          await this.callScriptFunc('onStateChanged', vmState);
        } catch (e) {
